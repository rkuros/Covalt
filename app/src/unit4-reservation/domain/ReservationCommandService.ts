...
export interface CreateReservationCommand {
  ...
  readonly displayName?: string; // 顧客操作時（自動登録用）
...
export class ReservationCommandService {
  ...
  async createReservation(
    ...
    // 3. スロットを予約確保（施術時間を渡して隣接スロットをブロック）
    const slotResult = await this.slotGateway.reserveSlot(
      slotId,
      reservationId,
  ...
  async modifyReservation(
    ...
    // 新スロットを先に予約確保（施術時間を渡して隣接スロットをブロック）
    const newSlotResult = await this.slotGateway.reserveSlot(
      newSlotId,
      reservationId,
    ...
    reservation.modify({
      newSlotId,
      newDateTime: ReservationDateTime.create(
        `${newSlotResult.date}T${newSlotResult.startTime}:00+09:00`,
      ),
  ...
  private async resolveCustomer(command: CreateReservationCommand): Promise<{
    ...
    if (command.createdBy === ActorType.Customer) {
      return this.resolveCustomerAsCustomer(command);
    }
    return this.resolveCustomerAsOwner(command);
  }

  /**
   * 顧客操作（LIFF 経由）: LINE ユーザーIDで顧客を特定しスナップショット情報を返す。
   */
  private async resolveCustomerAsCustomer(
    command: CreateReservationCommand,
  ): Promise<{
    customerId: string;
    customerName: string;
    ...
    const ownerId = OwnerId.create(command.ownerId);
    const customer = await this.customerGateway.findOrCreateByLineUserId(
      ownerId,
      lineUserId,
      command.displayName || command.lineUserId,
    );
    return {
  ...
  /**
   * オーナー操作（Web 管理画面経由）: 既存顧客選択 or 新規顧客作成でスナップショット情報を返す。
   */
  private async resolveCustomerAsOwner(
    command: CreateReservationCommand,
  ): Promise<{
    customerId: string;
    customerName: string;
    ...
    if (command.customerId) {
      ...
      throw new Error(
        'Either customerId or newCustomerName is required for owner operation',
      );
    }
    return {
      customerId: customer.customerId,
