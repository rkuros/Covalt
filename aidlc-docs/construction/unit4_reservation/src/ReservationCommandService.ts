...
export class ReservationCommandService {
  ...
  private async resolveCustomer(command: CreateReservationCommand): Promise<{
    ...
    // スロット情報を取得して dateTime, durationMinutes を確認
    const slotList = await this.slotGateway.findAvailableSlots(
      ownerId,
      '', // アプリケーション層で日付を指定する想定
    );
    const slot = slotList.slots.find((s) => s.slotId === slotId.value);
    if (!slot) {
      throw new Error(`Slot not found: ${slotId.value}`);
    }
    ...
    if (command.createdBy === ActorType.Customer) {
      ...
      return {
        ...
        ownerLineUserId: null, // アプリケーション層で別途取得する想定
