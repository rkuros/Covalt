...
import { GoogleCalendarApiClient, CalendarListEntry } from './GoogleCalendarApiClient';
...
export class CalendarIntegrationService {
   ...
   */
  async startOAuthFlow(ownerId: string): Promise<{ authorizationUrl: string; integrationId: string }> {
    ...
    const authorizationUrl = this.oauthService.buildAuthorizationUrl(integration.id);
   ...
   */
  async completeOAuthFlow(integrationId: string, authorizationCode: string): Promise<void> {
    const integration = await this.integrationRepository.findById(integrationId);
    if (!integration) {
      throw new Error(`連携設定が見つかりません: ${integrationId}`);
    }

    const token = await this.oauthService.exchangeAuthorizationCode(authorizationCode);
  ...
  async listCalendars(ownerId: string): Promise<CalendarListEntry[]> {
    ...
    return this.calendarApiClient.listCalendars(integration.oauthToken!.accessToken);
  }

  /**
   * BR-1.2: 連携先カレンダーを選択して連携を有効化する。
   */
  async selectCalendar(ownerId: string, calendarIdValue: string): Promise<void> {
    const integration = await this.getIntegrationWithValidToken(ownerId);
   ...
   */
  private async getIntegrationWithValidToken(ownerId: string): Promise<GoogleCalendarIntegration> {
    const integration = await this.integrationRepository.findByOwnerId(ownerId);
    if (!integration) {
      throw new Error(`オーナー ${ownerId} の連携設定が見つかりません`);
    }
    ...
    if (integration.oauthToken.needsRefresh()) {
      try {
        const refreshedToken = await this.oauthService.refreshToken(integration.oauthToken);
        integration.updateOAuthToken(refreshedToken);
        await this.integrationRepository.save(integration);
      } catch {
        integration.markRequiresReauth();
        await this.integrationRepository.save(integration);
        throw new Error('トークンのリフレッシュに失敗しました。再認証が必要です。');
      }
    }
